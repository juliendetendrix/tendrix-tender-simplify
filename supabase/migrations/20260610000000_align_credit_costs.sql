-- Alignement des coûts en crédits sur la grille de prix.
-- Analyse : 1 -> 50 crédits (5 €). Crédits offerts à l'inscription : 3 -> 50 (1 analyse).
-- (La réponse reste gérée par spend_credits(_amount) côté code : 3500 crédits.)

-- Crédits offerts à l'inscription (nouvelles entreprises)
alter table public.companies alter column credits set default 50;

-- Déduction de 50 crédits pour une analyse (au lieu de 1)
...
begin
  ...
  -- déduction atomique de 50 crédits
  ...
  values (_company_id, -50, 'analysis', v_analysis_id);

  return v_analysis_id;
end $$;

-- Remboursement de 50 crédits si l'analyse échoue (au lieu de 1)
...
begin
  ...
  update public.companies set credits = credits + 50 where id = _company_id;

  insert into public.credit_transactions (company_id, amount, reason, analysis_id)
  values (_company_id, 50, 'refund', _analysis_id);
end $$;
